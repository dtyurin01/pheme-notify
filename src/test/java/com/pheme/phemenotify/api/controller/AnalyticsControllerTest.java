...
class AnalyticsControllerTest {
    ...
    void shouldReturn200WithStats_whenValidDates() throws Exception {
        ...
        mockMvc.perform(get(DELIVERY_STATS_URL).param("startDate", START).param("endDate", END))
    ...
    void shouldReturn400_whenStartDateAfterEndDate() throws Exception {
        ...
        mockMvc.perform(get(DELIVERY_STATS_URL).param("startDate", END).param("endDate", START))
    ...
    void shouldReturn400_whenDateParamMissing() throws Exception {
        mockMvc.perform(get(DELIVERY_STATS_URL).param("startDate", START))
            .andExpect(status().isBadRequest());
    ...
    void shouldReturn400_whenDateFormatInvalid() throws Exception {
        mockMvc.perform(get(DELIVERY_STATS_URL).param("startDate", "not-a-date").param("endDate", END))
            .andExpect(status().isBadRequest());
