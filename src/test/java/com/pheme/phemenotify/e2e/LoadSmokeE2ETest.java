...
public class LoadSmokeE2ETest extends BaseIntegrationTest {
    ...
    private static final int SMS_LIMIT = 3; // application.yaml: notification.rate-limit.sms.max-requests
    ...
    void shouldProcessAllEvents_withoutLoss_whenBurstOf1000Sent() {
        userPreferenceRepository.save(PreferenceTestData
            .entityWith("user-load", Set.of(Channel.SMS), "en", "UTC"));

        for (int i = 0; i < EVENT_COUNT; i++) {
            NotificationEvent event = NotificationTestData.eventFor(
                "load-" + i, "user-load",
                Map.of("orderId", String.valueOf(i), "amount", "10"));
