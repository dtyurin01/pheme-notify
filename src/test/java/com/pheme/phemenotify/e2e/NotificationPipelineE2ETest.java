...
public class NotificationPipelineE2ETest extends BaseIntegrationTest {
  ...
  void shouldDeliverEmailNotification_whenValidEventReceived() {
    ...
    NotificationEvent event = NotificationTestData.defaultEvent();
    ...
    await()
        ...
        .untilAsserted(
            () -> {
              var found = notificationRepository.findByIdempotencyKey("event-1:EMAIL");
  ...
  void shouldDeliverToAllEnabledChannels_whenUserHasMultipleChannels() {
    ...
    userPreferenceRepository.save(
        PreferenceTestData.entityWith("user-2", Set.of(Channel.EMAIL, Channel.SMS), "en", "UTC"));

    NotificationEvent event =
        NotificationTestData.eventFor(
            "evt-h2",
            "user-2",
            Map.of("email", "user2@example.com", "orderId", "42", "amount", "100"));
  ...
  @Test
  void shouldSkipDuplicateEvent_whenSaveEventIdReceived() throws InterruptedException {
    // given
    userPreferenceRepository.save(
        PreferenceTestData.entityWith("user-3", Set.of(Channel.EMAIL), "en", "UTC"));
  ...
  void shouldSkipProcessing_whenUserHasNoPreferences() {
    NotificationEvent event =
        NotificationTestData.eventFor(
            "evt-p1",
            "user-4",
            Map.of("email", "user-4@example.com", "orderId", "1", "amount", "10"));
