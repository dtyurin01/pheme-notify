...
public class RateLimitE2ETest extends BaseIntegrationTest {
    ...
    void setUp() {
        ...
        var rateLimitKeys =
            stringRedisTemplate.keys("ratelimit:*");
    ...
    void shouldRespectRateLimit_whenBurstOfEventsReceived(){
        ...
        userPreferenceRepository.save(PreferenceTestData
            .entityWith("user-rl", Set.of(Channel.EMAIL), "en", "UTC"));

        // when - 7 event, different ids
        for (int i = 1; i <= 7; i++) {
            NotificationEvent event = NotificationTestData.eventFor(
                "evt-rl-" + i, "user-rl",
                Map.of("email", "user-rl@example.com",
                    "orderId", String.valueOf(i), "amount", "10")
            );
        ...
        await().atMost(30, TimeUnit.SECONDS)
            .untilAsserted(() -> {
                ...
                long delivered = notifications.stream()
