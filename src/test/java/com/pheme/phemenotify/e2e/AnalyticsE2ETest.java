...
public class AnalyticsE2ETest extends BaseIntegrationTest {
    ...
    void shouldReturnDeliveryStats_whenNotificationsExist() throws Exception {
        ...
        saveWithDate(notificationRepository, orderCompleted, daysAgo(1), Channel.EMAIL, NotificationStatus.DELIVERED, "key-1");
        saveWithDate(notificationRepository, orderCompleted, daysAgo(1), Channel.EMAIL, NotificationStatus.DELIVERED, "key-2");
        saveWithDate(notificationRepository, orderCompleted, daysAgo(1), Channel.EMAIL, NotificationStatus.FAILED,    "key-3");

        String startDate = LocalDate.now(ZoneOffset.UTC).minusDays(2).toString();
        String endDate = LocalDate.now(ZoneOffset.UTC).toString();

        // when & then
        mockMvc.perform(get("/api/v1/analytics/delivery-stats")
