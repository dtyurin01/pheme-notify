...
public class NotificationTestData {
  ...
  public static NotificationResponse responseWith(
      UUID id, NotificationStatus status, String errorMessage) {
    return new NotificationResponse(
        id,
        "user-1",
        Channel.EMAIL,
  ...
  public static NotificationEvent eventFor(String id, String userId, Map<String, String> payload) {
    return new NotificationEvent(
        id, userId, OrderCompletedEventType.CODE, Channel.EMAIL, payload, Instant.now());
  }

  public static NotificationEvent eventWithPayload(Map<String, String> payload) {
    return new NotificationEvent(
        "event-1", "user-1", OrderCompletedEventType.CODE, Channel.EMAIL, payload, Instant.now());
  }
}
