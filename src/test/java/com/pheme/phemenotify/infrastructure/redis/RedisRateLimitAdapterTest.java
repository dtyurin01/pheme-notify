...
public class RedisRateLimitAdapterTest extends BaseIntegrationTest {
  ...
  @Autowired private RedisRateLimitAdapter rateLimitAdapter;
  ...
  void shouldAllow_whenFirstRequest() {
    boolean allowed = rateLimitAdapter.isAllowed("user-1", "EMAIL", WINDOW_MS, MAX);
  ...
  void shouldBlock_whenLimitReached() {
    for (int i = 0; i < MAX; i++) {
      rateLimitAdapter.isAllowed("user-1", "EMAIL", WINDOW_MS, MAX);
    }

    boolean sixthRequest = rateLimitAdapter.isAllowed("user-1", "EMAIL", WINDOW_MS, MAX);
  ...
  void shouldIsolate_whenDifferentUsers() {
    ...
    boolean user2Allowed = rateLimitAdapter.isAllowed("user-2", "EMAIL", WINDOW_MS, MAX);
  ...
  void shouldIsolate_whenDifferentChannels() {
    ...
    boolean smsAllowed = rateLimitAdapter.isAllowed("user-1", "SMS", WINDOW_MS, MAX);
  ...
  void shouldAllow_whenWindowExpired() throws InterruptedException {
    ...
    for (int i = 0; i < MAX; i++) {
      rateLimitAdapter.isAllowed("user-1", "EMAIL", shortWindow, MAX);
    }
    assertThat(rateLimitAdapter.isAllowed("user-1", "EMAIL", shortWindow, MAX)).isFalse();
